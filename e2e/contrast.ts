 ...
 *
 *  - text over a surface axe declines to resolve. This lab's whole verdict
 *    vocabulary is built from them: `--ok-fill`, `--bad-fill` and
 *    `--warn-fill` are translucent `rgba()` washes, and they carry the
 *    accept / reject / unknown chips, the `.alarm` box, the `.callout-ok`
 *    box and the `.kv-evidence` rows in the inspector. `#app button.primary`
 *    and `[aria-pressed="true"]` are `color-mix(in srgb, …)` over whatever is
 *    behind them, and the hero aside is a `color-mix(in oklab, …)` again.
 *    Chromium hands every one of those back to `getComputedStyle` unresolved,
 *    so axe files the nodes under `incomplete` and a gate asserting only on
 *    `violations` never sees them at all — which is precisely the surface a
 *    security lab most needs measured, since the tint IS the verdict.
 ...
 *    `.cl-hero-sub` at `.85`. Disabled controls are dimmed by a colour swap
 *    (`#app button[disabled] { color: var(--text-dim) }`) rather than opacity,
 *    and are exempt as inactive components either way.
 *
 ...
 *    clip live text, and all three matter: `.diamond-scroll`, whose
 *    `min-width: 37rem` diagram is wider than a phone by design because the
 *    diamond shape is the lesson; `.step-log`, capped at 18rem and
 *    auto-scrolled to the newest line by `appendStep`; and `.der-dump`, capped
 *    at 14rem around a full hex dump of a certificate. Content scrolled past
 *    any of them is not dimmed or partly drawn: it is absent from the frame,
 *    and asking what colour it sits on has no answer. Its rect is still
 *    outside every ancestor's box, so the ancestor walk would find nothing
 *    behind it, fall through to white, and invent a failure. Skip it, and rely
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * `color-mix(in oklab, var(--accent) 6%, transparent)`, primary buttons
     * are `color-mix(in srgb, var(--accent) 18%, transparent)` and pressed
     * ones 22%, and Chromium reports every one of those to `getComputedStyle`
     * unchanged rather than converting it to sRGB. A regex that only
     * understands `rgb()/rgba()` sees `null` for each and the walk then falls
     * through to the wrong backdrop — which
     ...
     *
     * THIS LAB PAINTS NO GRADIENT TODAY, but it does paint a
     * `background-image`: `#app select` carries an inline `data:` SVG chevron.
     * That layer is not a gradient, so `sampleLayer` returns transparent for it
     * — correct, because the chevron is a 10x6 mark in the right padding and
     * paints nothing behind the option text. The sampler is kept for the
     * gradient case anyway, and not because it might be useful later:
     * `paintAt` has to decide what a `background-image` layer contributes, and
     * the alternative default — treat every layer as transparent — is silently
     * wrong the moment anyone gives a verdict chip or the diamond diagram a
     * gradient. The walk would then composite the text onto the surface BEHIND
     * the gradient and report a ratio nothing on screen has, in the direction
     * that hides failures.
     *
     ...
     *
     * A driven scan here walks a ten-row RFC 5280 check table (each row an
     * open `<details>`, a badge, an RFC reference and a paragraph of detail), a
     * nineteen-button inspector nav, a fourteen-row key/value table, a
     * twelve-term glossary, a step log that can run to dozens of entries and a
     * nine-node diagram — well over a thousand text-owning nodes, every one of
     * which re-walks the same handful of ancestors up to `<body>`. Without the
     * caches the pass re-reads the same computed styles and rects thousands of
     * times, and the gate scans two dozen states per configuration across four
     * configurations. Nothing mutates the DOM during the pass, so the cached
     ...
     * that is `.diamond-scroll`, `.step-log`, `.der-dump`, and `.sr-caption`,
     * whose `overflow: hidden` 1x1 box is how the check tables' captions are
     * exposed to a screen reader without being drawn.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. This lab is exactly that shape: `.diamond-svg` holds
      // nothing but `<line class="edge">` elements drawn as connectors, and
      // `.edge` sets only `stroke`, so `getComputedStyle().fill` reports the
      // initial black for every one of them.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // that case: the honest-scope note, the glossary, every stage hint and
      // one `<details>` per row of every RFC 5280 check table all arrive shut,
      // and the gate opens them by clicking their `<summary>` — the route a
      // reader has — rather than assigning `.open` from script, which is what
      // the previous gate did to all of them at once.
     ...
     * this page. Exactly two things carry it, and neither holds a character of
     * text: the shared header's hamburger and GitHub glyphs, and
     * `.diamond-svg`, which is the connector overlay — every node label in that
     * diagram is real HTML in `.diamond-grid` beside it, exposed and measured.
     */
     ...
     *
     * This lab's one SVG, `.diamond-svg`, holds only `<line>` connectors and no
     * `<text>`, so the guard is inert today. It stays because the mistake is
     * cheap to make and silent: the moment anyone labels an edge, a stray text
     * node dropped in a `<g>` or directly under the `<svg>` renders nothing but
     * measures as a 1:1 failure against the card — a ratio describing ink that
     * was never laid down.
     */
